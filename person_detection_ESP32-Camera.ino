...
void setup() {
  ...
  // NOLINTNEXTLINE(runtime-global-variables)
   static tflite::MicroMutableOpResolver<16> micro_mutable_op_resolver;
...
void loop() {
  ...
  uint8_t class0_score = output->data.uint8[0];//[kPersonIndex];
  uint8_t class1_score = output->data.uint8[1];//[kNotAPersonIndex];
  uint8_t class2_score = output->data.uint8[2];//[kPersonIndex];
  uint8_t class3_score = output->data.uint8[3];//[kNotAPersonIndex];
//  uint8_t class4_score = output->data.uint8[4];//[kNotAPersonIndex];
  error_reporter->Report("%d %d %d %d", class0_score, class1_score, class2_score, class3_score);
  RespondToDetection(error_reporter, class0_score, class1_score);
